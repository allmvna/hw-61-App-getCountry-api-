...
import {ALL_COUNTRY_URL, COUNTRY_DETAIL_URL, COUNTRY_FIELDS} from "./constants.ts";
...
const App = () => {
    ...
    const [detailCountry, setDetailCountry] = useState<DetailCountry | null>(null);
    ...
    const getCountry = async (alpha3Code: string) => {
        try {
            setLoading(true);
            const response = await axios<DetailCountry>((`${COUNTRY_DETAIL_URL}${alpha3Code}`));
            ...
            const borderNames = borderResponses.map((borderResponse) => borderResponse.data.name);
            ...
            <div className="container pt-3">
                <div className="row">
                    <div className="col-md-4">
                        <div className="countries-block" style={{ maxHeight: '650px', overflowY: 'auto' }}>
                            {loading ? (
                                <Loader />
                            ) : (
                                <CountryList countries={countries} onCountryClick={getCountry} />
                            )}
                        </div>
                    </div>
